      ...
      <header className="sticky top-0 z-30 w-full border-b bg-background/95 backdrop-blur-sm">
        <div className="flex h-14 items-center justify-between px-4 lg:px-6">
          ...
          <div className="hidden md:flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 px-3 py-1.5 rounded-full">
            <Clock size={14} />
            <span>Cập nhật: <span className="font-medium text-foreground">{mounted ? lastUpdated : '--:--:--'}</span></span>
          </div>

          {/* Right side - Actions */}
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={handleRefresh} className="gap-2 hover:bg-primary/10 hover:text-primary hover:border-primary/50 transition-all">
              <RefreshCcw className="h-4 w-4" />
              <span className="hidden sm:inline">Làm mới</span>
            </Button>
            <Button variant="outline" size="sm" className="gap-2 hover:bg-accent/20 hover:border-accent/50 transition-all hidden sm:flex">
              <Download className="h-4 w-4" />
              <span className="hidden lg:inline">Xuất báo cáo</span>
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setSettingsOpen(true)} className="hover:bg-muted hover:text-primary transition-colors hidden sm:flex">
